...
function Details(props) {
  ...
  return (
    <Card mb={6}>
      <CardContent>
        <Centered>
          <Avatar alt="Lucy Lavender" src={user.avatar_url} />
          <Typography variant="body2" component="div" gutterBottom>
            <Box fontWeight="fontWeightMedium">{ user.last_name } { user.first_name }</Box>
            <Box fontWeight="fontWeightRegular">{ user.bio }</Box>
          </Typography>
...
class UserProfile extends React.Component {
  ...
  componentDidMount() {
    const { user_id, session } = this.props;

    this.showUser(user_id || session.user.id);
  }
